...
class Cierre_model extends CI_Model{
    ...
    function consult_prod($fecha){
        ...
        $query = $this->db->query("SELECT SUM(tb_detalle_fac.MOV_CANT) AS CANT,tb_producto.PRO_NOMBRE as PROD_NOM ,tb_producto.PRO_PRECIOA FROM tb_detalle_fac,tb_factura,tb_producto WHERE tb_factura.ID_EMPRESA=".$this->session->userdata('id_empresa')." AND tb_factura.ID_FACTURA = tb_detalle_fac.ID_FACTURA and tb_producto.ID_PRODUCTO=tb_detalle_fac.ID_PRODUCTO AND DATE(tb_factura.FAC_FECHA)= '".$fecha."' group by tb_producto.PRO_NOMBRE");     
        return $query->result();
    }
